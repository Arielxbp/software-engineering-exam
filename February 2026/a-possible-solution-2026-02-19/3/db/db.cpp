...
void DB::init() {
 ...
 for (i=0; i < p -> prm.P; i++)
  ...
  time_of_next_event = p -> prm.l + p -> prm.s;
...
void DB::receive(double present_time)
{
...
#if (DEBUG > 1000)
   fprintf(stderr, "%lf: %lf, %s -> %s, msg: %s [DB::receive()]\n",
  ...
  // read message with format: reqid customerid item amount
	...
	switch (reqid)
	  ...
	  sprintf(msg2send.sender,"%s", sysname);
	  sprintf(msg2send.receiver,"%s", m.sender);
	  sprintf(msg2send.msg, "2 %d %d %d", transaction, item, amount);
 ...
 // read message with format: item amount
	...
	// send ack
	  ...
	  sprintf(msg2send.sender,"%s", sysname);
	  sprintf(msg2send.receiver,"%s", m.sender);
	  sprintf(msg2send.msg, "%d %d", item, amount);
