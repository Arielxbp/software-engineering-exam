...
{ 
  ...
  fprintf(stderr, "System::create(%d): begin: C = %d, S = %d, F = %d\n",
	  dbg_counter, p -> prm.C, p -> prm.S, p -> prm.F);
#endif
...
#if (DEBUG > 100)
  fprintf(stderr, "System::create(%d): monitor created\n", dbg_counter);
#endif
  ...
  // Create Channels
  p -> Chp2n.resize(p -> first_available_pid);
  p -> Chn2p.resize(p -> first_available_pid);
  
  //  p -> Chp2n = new ChMsg[p -> num_processes];
  //  p -> Chn2p = new ChMsg[p -> num_processes];

  for (int i = 0; i < p -> first_available_pid; ++i)
    {
      p -> Chp2n[i] = make_unique<ChMsg>();
      p -> Chn2p[i] = make_unique<ChMsg>();
   }
  
  
#if (DEBUG > 1000)
  fprintf(stderr, "System::create(%d): channels created\n", dbg_counter);
#endif
...
#if (DEBUG > 100)
  fprintf(stderr, "System::create(%d): network created, p -> num_processes = %d\n",
	  dbg_counter, p -> num_processes);
  dbg_counter++;
#endif
...
#if (DEBUG > 1000)
  fprintf(stderr, "System::init(): create done\n");
#endif
...
#if (DEBUG > 1000)
  fprintf(stderr, "System::init(): define time\n");
#endif
