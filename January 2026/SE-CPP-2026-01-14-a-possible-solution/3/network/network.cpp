...
#if (DEBUG > 1000)
  fprintf(stderr, "Network::Network(%s, %d)\n", mysysname, pid);
#endif
...
#if (DEBUG > 1000)
  fprintf(stderr, "Network::scan(): begin: Network moves, time  = %lf\n", present_time);
#endif
      ...
      // queue nonempty
   {
      ...
      assert(p -> dns[m.sender] == scanner[scanned]);
      
      p -> Chn2p[p -> dns[m.receiver]].push(m);
   }

   scanned++;

#if (DEBUG > 1000)
  fprintf(stderr, "Network::scan(): end: Network moves, time  = %lf\n", present_time);
#endif
