...
public:
    CustomerProcess(int pid, int S, int P, int Q, double A, double B, MessageBus &bus, RandomGenerator &rng, double S0, double P0, double Q0, int serverBase)
        : pid_(pid), S_(S), P_(P), Q_(Q), A_(A), B_(B), bus_(bus), rng_(rng), S0_(S0), P0_(P0), Q0_(Q0), serverBase_(serverBase) {}
    ...
    void handleEvent(double currentTime) override {
        ...
        m.time = currentTime;
        m.sender = pid_;
        m.receiver = serverBase_ - 1 + (rng_.bernoulli(S0_) ? pickEven(S_, rng_) : pickOdd(S_, rng_));
...
public:
    ServerProcess(int pid, int C, int F, int databasePid, int P, int Q, MessageBus &bus, RandomGenerator &rng, double ST0, double ST1, double SEP, double SOP, double QEA, double QOA, double T, std::vector<int> &costs, std::vector<double> &rr, double serverReadDelay = 0.0, double serverWriteDelay = 0.0)
        : pid_(pid), C_(C), F_(F), databasePid_(databasePid), P_(P), Q_(Q), bus_(bus), rng_(rng), ST0_(ST0), ST1_(ST1), SEP_(SEP), SOP_(SOP), QEA_(QEA), QOA_(QOA), T_(T), costs_(costs), rr_(rr), serverReadDelay_(serverReadDelay), serverWriteDelay_(serverWriteDelay), sellBuy_(0.0), y_(0.0), updateYTime_(0.0) {
    }
    ...
    void handleEvent(double currentTime) override {
        ...
        if (currentTime >= updateYTime_) {
            ...
            y_ = y_ + T_ * 0.1 * sum;
            rr_[pid_ - C_ - F_] = (sellBuy_ - y_)/updateYTime_;
...
private:
    ...
    int pid_, C_, F_, databasePid_, P_, Q_;
    MessageBus &bus_;
    RandomGenerator &rng_;
...
double runMsims(double H, int M, int C, int S, int F, double A, double B, double V, double W, int P, int Q, double r, double T, double S0, double P0, double Q0, double ST0, double ST1, double SEP, double SOP, double QEA, double QOA, SELib::RandomGenerator &rng) {
    ...
    for (int m = 0; m < M; ++m) {
        ...
        for(int i=0; i<C; ++i) {
            system.emplaceProcess<SELib::CustomerProcess>(i, S, P, Q, A, B, bus, rng, S0, P0, Q0, C + F);
        }
        for(int i=0; i<F; ++i) {
            system.emplaceProcess<SELib::SupplierProcess>(C+i, databasePid, P, Q, V, W, bus, rng);
        }
        for(int i=0; i<S; ++i) {
            system.emplaceProcess<SELib::ServerProcess>(C + F + i, C, F, databasePid, P, Q, bus, rng, ST0, ST1, SEP, SOP, QEA, QOA, T, costs, rr, 0, 0); // (serverReadDelay, serverWriteDelay)
        }
...
int main() {
    ...
    double best_QEA = QEA;
    double best_QOA = QOA;
    std::vector<std::tuple<double,double,double,double>> best_SEP_SOP_QEA_QOA;
    for (int k = 0; k <= G; ++k) {
        SEP =(double) k / G;
        for (int kk = 0; kk <= G; ++kk) {
            ...
            for (int kkk = 0; kkk <=G; ++kkk) {
                QEA = (double) kkk / G;
                for (int kkkk = 0; kkkk <=G; ++kkkk) {
                    QOA = (double) kkkk / G;
                    printf("Testing parameters: SEP=%f, SOP=%f, QEA=%f, QOA=%f\n", SEP, SOP, QEA, QOA);
                    double rr = SELib::runMsims(H, M, C, S, F, A, B, V, W, P, Q, r, T, S0, P0, Q0, ST0, ST1, SEP, SOP, QEA, QOA, rng);
                    if (rr > max_rr) {
                        max_rr = rr;
                        best_SEP_SOP_QEA_QOA.clear();
                        best_SEP_SOP_QEA_QOA.emplace_back(SEP, SOP, QEA, QOA);
                    } else if (rr == max_rr) {
                        best_SEP_SOP_QEA_QOA.emplace_back(SEP, SOP, QEA, QOA);
                    }
                }
    ...
    outFile << "SEP: " << std::fixed << std::setprecision(6) << best_SEP << std::endl;
    outFile << "SOP: " << std::fixed << std::setprecision(6) << best_SOP << std::endl;
    outFile << "QEA: " << std::fixed << std::setprecision(6) << best_QEA << std::endl;
    outFile << "QOA: " << std::fixed << std::setprecision(6) << best_QOA << std::endl;
